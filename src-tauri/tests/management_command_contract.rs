use prelay_client::commands::{endpoints::EndpointSaveInput, providers::ProviderSaveInput};
use prelay_protocol::{RotateCredentialRequest, TestProviderProtocolRequest};
...
fn management_command_inputs_never_accept_identity_or_device_credentials() {
    ...
    let endpoint = serde_json::to_value(EndpointSaveInput::default()).expect("serialize endpoint");
    ...
    for value in [provider, endpoint, protocol] {
