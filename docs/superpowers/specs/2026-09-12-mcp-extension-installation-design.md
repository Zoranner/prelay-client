...
安装界面按清单逐项生成输入项（当前为逐项密码输入框），不把变量拼成一段文本。

当前实现中，输入取值只存在于界面状态：安装请求只携带扩展包、目标客户端和覆盖标记。取值不写入宿主配置，宿主配置写入的是环境变量名称引用：
...
直接在界面输入并保存密钥值，需要额外接入 Windows Credential Manager 或其他本地安全凭据存储，不属于当前安装模型。
...
参数是否包含用户目录变量不影响展示，安装时按清单原样写入宿主配置。
...
当前用于联调的本地模拟入口仍然保留，只属于开发测试入口：

- MCP 分类右上角的临时「安装」按钮调用 `extensions_mcp_test_package`，返回一个模拟 MCP 包；
- 不请求服务端；
- 复用真实预览和宿主配置写入流程；
- 真实扩展链路（真实 MCP 包加服务端）验证通过后从正式产品入口移除。
